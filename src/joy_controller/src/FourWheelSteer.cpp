...
bool FourWheelSteer::TurnRadiusLimitter(double &TurnRadius)
{
    ...
    if (abs(TurnRadius) < TurnRadius_min)
        TurnRadius = TurnRadius_min;
...
void FourWheelSteer::xVehicle(double vx, double w)
{
    ...
    if (w > 0)
    {
        Angle[0] = inside_angle;
        Angle[1] = -inside_angle;
        Angle[2] = -outside_angle;
        Angle[3] = outside_angle;
        AngVel[0] = AngVel[1] = inside_angVel;
        AngVel[2] = AngVel[3] = outside_angVel;
    }
    else
    {
        Angle[0] = -outside_angle;
        Angle[1] = outside_angle;
        Angle[2] = inside_angle;
        Angle[3] = -inside_angle;
...
void FourWheelSteer::calcOdom(double angVel[4], double angle[4])
{
    ...
    theta += ROOT2 * (vel[0] * (-cos(angle[0]) + sin(angle[0])) + vel[1] * (-cos(angle[1]) - sin(angle[1])) + vel[2] * (cos(angle[2]) - sin(angle[2])) + vel[3] * (cos(angle[3]) + sin(angle[3]))) / 8.0 / DistWheelCenter * dt;
}
